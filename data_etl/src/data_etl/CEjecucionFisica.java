...
public class CEjecucionFisica {
	...
	public static boolean loadEjeucionHoja(Connection conn, boolean historico, String schema){
		...
		String query = "select aprobado, clase_modificacion, cuatrimestre, descripcion, documento_gestion, ejercicio, entidad, estado, "
				...
				+ "no_documento, no_secuencia, solicitado, tipo_documento, unidad_ejecutora FROM "+schema+".sf_ejecucion_hoja_4 "+
		...
		try{
			if(!conn.isClosed()){
				...
				if(rs!=null && bconn){
					...
					while(rs.next()){
						if(first){
							PreparedStatement pstm1 = CMemSQL.getConnection().prepareStatement("delete from ejecucion_hoja "
									+ (!historico ? " where ejercicio=" + date.getYear() : ""))  ;
	...
	public static boolean loadEjecucionDetalle(Connection conn, boolean historico, String schema){
		DateTime date = new DateTime();
		String query = "select actividad, aprobado, cantidad_unidades, codigo_meta, ejercicio, entidad, fecha_aprobado, fecha_solicitado, "
				...
				+ "subprograma, unidad_ejecutora, unidad_ejecutora_origen FROM "+schema+".sf_ejecucion_detalle_4 "+
				(!historico ? "	   WHERE ejercicio = " + date.getYear() + " " : "" );
		boolean ret = false;
		try{
			if(!conn.isClosed()){
